...
class Device:
    ...
    def velocity_distance(self):
        ...
        v = np.array(v)
        velocity = v.mean()
        return velocity, reading_next['distance']
    ...
    def _is_outlier(self, list_reading):
        ...
        if self._last_good_reading == None:
            if abs(accel_x) > 14000 or abs(accel_y) > 14000:
                print('outlier: {}\t'.format(list_reading))
                return True
        else:
            ...
            if abs(last_good_accel_x - accel_x) > CHANGE_THRESHOLD or abs(last_good_accel_y - accel_y) > CHANGE_THRESHOLD:
                is_outlier = True
                print('outlier: {}\tprev: {}'.format(list_reading, self._last_good_reading))
        if not is_outlier:
...
if __name__ == '__main__':
    ...
    while True:
        v, d = device.velocity_distance()
        print('velocity: {}\tdistance: {}'.format(v, d))
        time.sleep(1)
aS
    # device.terminate()
